<!-- Stocuri BO2 (placeholder) -->
<section x-show="tab === 'stocuri'" x-transition>
    <h2 class="text-2xl font-bold text-gray-800 text-center mb-6">Stocuri BO2</h2>
    <div class="overflow-x-auto mb-6">
        <table class="min-w-full bg-white shadow rounded-lg">
            ...
            <tbody>
                <?php foreach ($stocuri_bo2 as $row): ?>
                <tr class="border-t">
    ...
    <section class="mt-12 mb-8">
        ...
        <table class="min-w-full bg-white shadow rounded-lg">
            ...
            <tbody>
                <?php
            $activitati_bo2 = $pdo->query("
                SELECT a.*, m.denumire,
                    u.first_name AS user_first, u.last_name AS user_last, u.role AS user_role,
                    med.first_name AS medic_first, med.last_name AS medic_last
                FROM activitate_stoc_bo2 a
                JOIN medicamente m ON m.id = a.medicament_id
                LEFT JOIN users u ON u.id = a.realizat_de
                LEFT JOIN users med ON med.id = a.solicitat_de
                ORDER BY a.created_at DESC
            ")->fetchAll();

            foreach ($activitati_bo2 as $a): ?>
                <tr class="border-t">
                    <td class="px-4 py-2 text-sm text-gray-600"><?= date('d.m.Y H:i', strtotime($a['created_at'])) ?>
                    </td>
